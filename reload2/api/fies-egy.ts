...
export default async function handler() {
  try {
    const r = await fetch('https://ourworldindata.org/grapher/share-of-population-with-moderate-or-severe-food-insecurity.csv',
      { headers:{accept:'text/csv','user-agent':'Mozilla/5.0 VercelEdge/1.0'} });
    if (!r.ok) return err(502, `OWID fies ${r.status}`);
  ...
  } catch(e:any){ return err(500, `fies error: ${String(e)}`); }
}
